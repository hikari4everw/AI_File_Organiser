...
- 可主动下载约 173 MB 的本地图像模型，为 PDF、EPUB、图片和有界目录样本生成相似概念候选；新文件的相似候选目前均需人工确认。
...
概念模型只在用户点击“下载本地图像模型”后从 Apple 发布的固定版本下载，并在安装前校验每个文件的大小和 SHA-256。未安装模型时仍可手动记录概念和明确标签，但不会为新文件生成图像相似候选。自动概念确认尚未开放：真实留出样本还没有达到约定的完整验收条件，所有相似候选都留在审核中。
