...
public actor NativeContentExtractor: ContentExtractor {
  ...
  public func extractContext(for item: ItemSnapshot) async -> ExtractedContext {
    guard item.kind == .file, !item.isCloudPlaceholder else { return .init() }
  ...
  private func extractPDF(_ url: URL) -> ExtractedContext {
    guard let document = PDFDocument(url: url), let page = document.page(at: 0),
      let text = page.string
    else {
      return .init(source: "unreadable-pdf")
    }
    return clipped(text, source: "pdf-first-page", inputWasLimited: document.pageCount > 1)
  }

  private func extractImageText(_ url: URL) -> ExtractedContext {
    guard let image = NSImage(contentsOf: url) else { return .init(source: "unreadable-image") }
    var proposedRect = NSRect(origin: .zero, size: image.size)
    guard let cgImage = image.cgImage(forProposedRect: &proposedRect, context: nil, hints: nil)
    else {
      return .init(source: "unreadable-image")
    }
    ...
    do {
      ...
      let text = (request.results ?? []).compactMap { $0.topCandidates(1).first?.string }.joined(
        separator: "\n")
      return clipped(text, source: "vision-ocr", inputWasLimited: false)
    } catch {
      return .init(source: "ocr-failed")
    }
  }
